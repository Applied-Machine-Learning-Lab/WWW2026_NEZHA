...
sft_instruct = "The user has interacted with items {} in chronological order. Can you predict the next item?"
...
class SeqRecDataset(BaseDataset):
    ...
    def _load_data(self):
        with open(os.path.join(self.data_path,f"inter.json"), 'r') as f:
    ...
    def _process_train_data(self):
        ...
        for uid in self.remapped_inters:
            items = self.remapped_inters[uid][:-2]  # input of each training sample
            if len(items)>1:
                for i in range(1, len(items)):
                    ...
                    if self.args.use_mtp:
                        ...
                        his = ", ".join(history)
                        instruction = sft_instruct.format(his)
